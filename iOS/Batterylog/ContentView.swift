...
struct ContentView: View {
    ...
    @State private var editingEntry: BatterylogEntry?
                        ...
                        ForEach(store.entries) { entry in
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    if !entry.voltage.isEmpty {
                                        Text("Voltage: \(entry.voltage)")
                                            .font(.caption)
                                            .foregroundStyle(Theme.accent)
                                    }
                                }
...
enum EntryFormMode {
    case add
    case edit(BatterylogEntry)
}

struct EntryFormView: View {
    let mode: EntryFormMode
    var onSave: (BatterylogEntry) -> Void
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Voltage", text: $field2)
                        .accessibilityIdentifier("field2Field")
                        .focused($focusedField, equals: .field2)
                    TextField("Brand", text: $field3)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry: BatterylogEntry
                        if case .edit(let existing) = mode {
                            entry = existing
                        } else {
                            entry = BatterylogEntry(title: "")
                        }
                        entry.title = title.isEmpty ? "Untitled" : title
                        entry.date = date
                        entry.voltage = field2
                        entry.brand = field3
                        entry.note = note
                        onSave(entry)
                        dismiss()
            ...
            .onAppear {
                if case .edit(let existing) = mode {
                    ...
                    date = existing.date
                    field2 = existing.voltage
                    field3 = existing.brand
                    note = existing.note
                }
            }
        }
