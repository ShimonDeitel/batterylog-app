...
final class StoreTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        let ok = store.add(BatterylogEntry(title: "Test"))
        XCTAssertTrue(ok)
    ...
    func testAddFailsAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(BatterylogEntry(title: "Filler"))
        }
        let result = store.add(BatterylogEntry(title: "Overflow"))
        XCTAssertFalse(result)
    }

    func testDeleteRemovesEntry() {
        let store = Store()
        _ = store.add(BatterylogEntry(title: "ToDelete"))
    ...
    func testUpdateChangesTitle() {
        let store = Store()
        _ = store.add(BatterylogEntry(title: "Original"))
    ...
    func testDeleteAtOffsets() {
        let store = Store()
        let before = store.entries.count
        _ = store.add(BatterylogEntry(title: "OffsetTest"))
